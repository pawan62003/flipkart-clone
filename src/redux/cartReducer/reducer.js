	...
	cartData: [
		{
			...
			price: 20,
			poster:
				"https://www.bigbasket.com/media/uploads/p/l/10000200-5_1-fresho-tomato-hybrid.jpg",
		},
		{
			title: "Tomatoes",
			qty: 1,
			categories: "vegetable",
			defaultImages:
				"https://png.pngtree.com/png-vector/20210529/ourmid/pngtree-tomato-food-gourmet-vegetables-png-image_3369387.jpg",
			id: 2113,
			price: 25,
			poster:
				"https://www.bigbasket.com/media/uploads/p/l/10000200-5_1-fresho-tomato-hybrid.jpg",
...
export const reducer = (state = initalCart, { type, payload }) => {
	...
	switch (type) {
		...
		case INCREMENT_PRODUCT: {
			const incProduct = state.cartData.map((item) =>
				item.id === payload ? { ...item, qty: item.qty + 1 } : item
			);
			return { ...state, cartData: incProduct };
		}
		case DECREMENT_PRODUCT: {
			const decProduct = state.cartData.map((item) =>
				item.id === payload ? { ...item, qty: item.qty - 1 } : item
			);
		...
		case CONFORM_ORDER: {
			return {
				...state,
				cartData: [],
				cartHistory: [...state.cartData, ...state.cartHistory],
			};
		}
